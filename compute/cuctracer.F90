SUBROUTINE CUCTRACER &
 ...
 & PMFUUS,   PMFDUS,   PMFU,     PMFD,  PUDRATE,  PDDRATE,&
 & PDMFUP,   PDMFDP,&
 & PCEN,     PTENC,    PSCAV  )  
...
!      ------------------------------------------------------------------
!     KCTOP IS FIRST LEVEL FROM TOP WHERE PMFUUS>0      
!     KDTOP IS FIRST LEVEL FROM TOP WHERE PMFDUS<0      
...
!     LDCUM  IS TRUE IF CONVECTION EXISTS, i.e. IF PMFUUS>0 IN COLUMN OR IF
!                       KCTOP>0 AND KCTOP<KLEV
!     LDDRAF IS TRUE IF DOWNDRAUGHTS EXIST IF PMFDUS<0 IN COLUMN OR IF
  ...
  DO JK=KLEV-1,3,-1
    ...
    DO JL=KIDIA,KFDIA
      IF ( LLCUMASK(JL,JK) ) THEN
        ZERATE=PMFUUS(JL,JK)-PMFUUS(JL,IK)+PUDRATE(JL,JK)
        ZMFA=1.0_JPRB/MAX(ZRMFCMIN,PMFUUS(JL,JK))
        IF (JK >=KCTOP(JL) )  THEN
          ZCU(JL,JK,JN)=( PMFUUS(JL,IK)*ZCU(JL,IK,JN)+ZERATE*PCEN(JL,JK,JN) &
  ...
  DO JK=3,KLEV
    ...
    DO JL=KIDIA,KFDIA
        ...
        ELSEIF ( LDDRAF(JL).AND.JK>KDTOP(JL) ) THEN
             ZERATE=-PMFDUS(JL,JK)+PMFDUS(JL,IK)+PDDRATE(JL,JK)
             ZMFA=1._JPRB/MIN(-ZRMFCMIN,PMFDUS(JL,JK))
             ZCD(JL,JK,JN)=( PMFDUS(JL,IK)*ZCD(JL,IK,JN)-ZERATE*PCEN(JL,IK,JN) &
...
! In order to avoid negative Tracer at KLEV adjust ZCD
  ...
  DO JL=KIDIA,KFDIA
    IF (LDDRAF(JL)) THEN
     ZPOSI=-ZDP(JL,JK)*(PMFUUS(JL,JK)*ZCU(JL,JK,JN)+PMFDUS(JL,JK)*ZCD(JL,JK,JN)&
                       &-(PMFUUS(JL,JK)+PMFDUS(JL,JK))*PCEN(JL,IK,JN) )
     IF( PCEN(JL,JK,JN)+ZPOSI*PTSPHY<0.0_JPRB ) THEN
        ZMFA=1._JPRB/MIN(-ZRMFCMIN,PMFDUS(JL,JK))
        ZCD(JL,JK,JN)=( (PMFUUS(JL,JK)+PMFDUS(JL,JK))*PCEN(JL,IK,JN)-PMFUUS(JL,JK)*ZCU(JL,JK,JN)&
...
!                  --------------
    ...
    DO JK=2,KLEV
      ...
      DO JL=KIDIA,KFDIA
        IF(LLCUMASK(JL,JK)) THEN
          ZMFA=PMFUUS(JL,JK)+PMFDUS(JL,JK)
          ZMFC(JL,JK,JN)=PMFUUS(JL,JK)*ZCU(JL,JK,JN)+PMFDUS(JL,JK)*ZCD(JL,JK,JN)&
      ...
      DO JK=2,KLEV
        ...
        ! LLCUMBAS(JL,JK)=LLCUMASK(JL,JK).AND.JK<=KCBOT(JL)
          ...
          IF(LLCUMBAS(JL,JK)) THEN
            ...
            ZMFC(JL,JK,JN)=-ZZP*(PMFUUS(JL,JK)+PMFDUS(JL,JK))
            IF(JK<KLEV) THEN
