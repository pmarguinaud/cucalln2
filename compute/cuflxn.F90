...
!OCL NOVREC
  ...
  ENDDO
ENDDO
PFPLCL(:,KLEV+1)=0.0_JPRB
PFPLCN(:,KLEV+1)=0.0_JPRB
PFPLCL(:,1)=0.0_JPRB
PFPLCN(:,1)=0.0_JPRB
PSNDE(:,1,1)=0.0_JPRB
PSNDE(:,1,2)=0.0_JPRB
...
DO JK=KTOPM2,KLEV
  DO JL=KIDIA,KFDIA
    IF(LDCUM(JL).AND.JK >= KCTOP(JL)-1) THEN
      ...
      IF(PFPLCN(JL,JK) > 0.0_JPRB.AND.ZTEN > RTT) THEN
        ZCONS1=ZCONS1A*(1.0_JPRB+0.5_JPRB*(ZTEN-RTT))
        ZFAC=ZCONS1*(PAPH(JL,JK+1)-PAPH(JL,JK))
        ZSNMLT=MIN(PFPLCN(JL,JK),ZFAC*(ZTEN-RTT))
        PDPMEL(JL,JK)=ZSNMLT
        IF (LPHYLIN) THEN
...
!     add heat not yet liberated in updraught precip
      ...
      ENDIF
      PFPLCL(JL,JK+1)=PFPLCL(JL,JK)+ZALFAW*&
       & (PDMFUP(JL,JK)+PDMFDP(JL,JK))+PDPMEL(JL,JK)-PSNDE(JL,JK,1)
      PFPLCN(JL,JK+1)=PFPLCN(JL,JK)+(1.0_JPRB-ZALFAW)*&
       & (PDMFUP(JL,JK)+PDMFDP(JL,JK))-PDPMEL(JL,JK)-PSNDE(JL,JK,2)
      IF(PFPLCL(JL,JK+1)+PFPLCN(JL,JK+1) < 0.0_JPRB) THEN
        PDMFDP(JL,JK)=-(PFPLCL(JL,JK)+PFPLCN(JL,JK)+PDMFUP(JL,JK))
        PFPLCL(JL,JK+1)=0.0_JPRB
        PFPLCN(JL,JK+1)=0.0_JPRB
        PDPMEL(JL,JK)  =0.0_JPRB
      ELSEIF(PFPLCL(JL,JK+1) < 0.0_JPRB ) THEN
        PFPLCN(JL,JK+1)=PFPLCN(JL,JK+1)+PFPLCL(JL,JK+1)
        PFPLCL(JL,JK+1)=0.0_JPRB
      ELSEIF(PFPLCN(JL,JK+1) < 0.0_JPRB ) THEN
        PFPLCL(JL,JK+1)=PFPLCL(JL,JK+1)+PFPLCN(JL,JK+1)
        PFPLCN(JL,JK+1)=0.0_JPRB
      ENDIF
    ENDIF
  ENDDO
...
DO JK=KTOPM2,KLEV
  DO JL=KIDIA,KFDIA
    IF(LDCUM(JL).AND.JK >= KCBOT(JL)) THEN
      ZRFL=PFPLCL(JL,JK)+PFPLCN(JL,JK)
      IF(ZRFL > 1.E-12_JPRB) THEN
        ...
        ZDENOM=1.0_JPRB/MAX(1.E-12_JPRB,PFPLCL(JL,JK)+PFPLCN(JL,JK))
        PFPLCL(JL,JK+1)=PFPLCL(JL,JK)+ZPDR&
         & +PDPMEL(JL,JK)+ZDRFL*PFPLCL(JL,JK)*ZDENOM  
        PFPLCN(JL,JK+1)=PFPLCN(JL,JK)+ZPDS &
         & -PDPMEL(JL,JK)+ZDRFL*PFPLCN(JL,JK)*ZDENOM  
        PDMFUP(JL,JK)=PDMFUP(JL,JK)+ZDRFL
        IF(PFPLCL(JL,JK+1)+PFPLCN(JL,JK+1) < 0.0_JPRB) THEN
          PDMFUP(JL,JK)=PDMFUP(JL,JK)-(PFPLCL(JL,JK+1)+PFPLCN(JL,JK+1))
          PFPLCL(JL,JK+1)=0.0_JPRB
          PFPLCN(JL,JK+1)=0.0_JPRB
          PDPMEL(JL,JK)  =0.0_JPRB
        ELSEIF(PFPLCL(JL,JK+1) < 0.0_JPRB ) THEN
